...
variable "max_count" {
  type        = number
  description = "The max number of tasks that should be running"
}

variable "min_count" {
  type        = number
  description = "The min number of tasks that should be running"
}
...
variable "memory_usage_scaling_trigger" {
  type        = number
  description = "If the average memory utilization rises over this percentage, the service will spin up more tasks to work (up to the defined max count). And if the utilization is constantly below the target, the service will deregister tasks (down to the defined min count)"
}

variable "cpu_usage_scaling_trigger" {
  type        = number
